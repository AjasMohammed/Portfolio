...
/* `size` takes a number (px) or any CSS length (e.g. a clamp()) — sized via
   style so responsive values work; numbers behave exactly as before. */
export function SocialIcon({ name, size = 18 }: { name: string; size?: number | string }) {
  const stroke = "currentColor";
  const sw = 1.6;
  switch (name) {
    case "github":
      return (
        <svg style={{ width: size, height: size }} viewBox="0 0 24 24" fill={stroke} aria-hidden>
    ...
    case "mail":
      return (
        <svg style={{ width: size, height: size }} viewBox="0 0 24 24" fill="none" stroke={stroke} strokeWidth={sw} strokeLinecap="round" strokeLinejoin="round" aria-hidden>
